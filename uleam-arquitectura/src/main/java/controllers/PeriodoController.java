...
public class PeriodoController implements Serializable {
	...
	static	Periodo p;
	public static  Periodo getMaxPeriodo() {		
		JPAService.runInTransaction(em ->{
			Query query = em.createQuery("select p from Periodo p where p.idPeriodo = (select max(idPeriodo) from Periodo)");

			if(query.getResultList().size()>0) {
				p = (Periodo) query.getSingleResult();
			}
